...
fun UpcomingMoviesScreen(
    ...
    if (movieListUiState.popularMovieList.isEmpty()) {
        ...
        LazyVerticalGrid(
            ...
            items(movieListUiState.upcomingMovieList.size) { index ->
                MovieItem(
                    movie = movieListUiState.upcomingMovieList[index],
                    navController = navController
                )
                Spacer(modifier = Modifier.height(16.dp))
                if (index >= movieListUiState.upcomingMovieList.size - 1 && !movieListUiState.isLoading) {
